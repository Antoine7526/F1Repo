...
ui <- dashboardPage(
  ...
  header = dashboardHeader(title = tags$img(src='logoF1.png', height = '40', width ='160'),
                           dropdownMenu(type = "messages", 
                                        ...
                                        badgeStatus =NULL,
                                        headerText = ("Dernière mise à jour réalisée le 17 nov 2021"))
  ),
  #Tableau de gauche
  sidebar = dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Maps", tabName = "Maps",icon = icon("map-pin")),
      hr(),
    ...
    tabItems(
      tabItem(tabName = "Accueil", fluidRow(column(width = 6,  align = "center", tags$div(tags$br(),tags$br(),tags$br(), tags$h1("Bienvenue sur notre site"),tags$br(),tags$br(),tags$br(),tags$br(),tags$div(style= "font-weight: 1000; font-family: DejaVu Sans Mono, monospace", "Si vous vous posez des questions sur la Formule 1 vous êtes au bon endroit.Vous trouverez ici différentes analyses graphiques (bien entendu expliquées) reprennant les données depuis 1950 jusqu'à 2020."),tags$br(),tags$br(),tags$br(),tags$br()),
                                                   fluidRow(column(width= 6, tags$img(src='logoblanc.png', height = '125', width ='203')),
                                                            column(width= 6, tags$div(style= "font-weight: 1000; font-family: DejaVu Sans Mono, monospace","Cette application a été réalisée dans le cadre d'un projet demandé au sein du M1 Statistiques et Sciences des Données de l'Université Grenoble Alpes", tags$br(),tags$br(),tags$br(),tags$br()))),
                                                   fluidRow(column(width = 4,  align = "center",tags$div(style= "font-weight: 1000; font-family: DejaVu Sans Mono, monospace",tags$br(),tags$br(),"Réalisée par:", tags$br(), "Batiste Amistadi, Charline Champ et Antoine Grancher")),
      ...
      tabItem(tabName = "Maps",fluidRow(column(width = 12, box(title = "Année", width = NULL, solidHeader = TRUE, status = "info",style="circle",selectInput("anneemap", label = NULL,choices = seq(2020,1950, by =-1)),selected ="choisir"))),
              fluidRow(column(width = 12, box(title = "MAP",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,leafletOutput("mymap"))))
      ),
