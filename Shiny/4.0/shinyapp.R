...
g1 <- "Sur votre gauche, vous remarquerez un diagramme en barres. Ce diagramme représente la fréquence de la position d'arrivée du pilote ayant fait le tour le plus rapide. On s'attend à ce pilote soit dans les premières positions, or ce n'est pas toujours le cas. Les pilotes ayant fait le tour le plus rapide finissent relativement dans le TOP 10. Il se peut que lors d'un grand prix un pilote ayant eu le meilleur tour ne finisse pas la course. Ainsi, cela sera matérialisé par une position égale à 0."   
...
g3 <- "Ce diagramme en barres représente le nombre de points moyens par pilote sur une saison. Remarquons qu'à l'aide de ce graphique nous pouvons retrouver un semblant du classement final des pilotes. La barre la plus haute représentant le leader."
...
g7 <- "Ce diagramme circulaire représente le pourcentage d'abandons pour chaque écurie lors d'une saison. Nous considérons comme abandon tous ceux n'ayant pas pris le départ (DNS) mais également ceux qui n'ont pas terminé la course (DNF)."
g8 <- "Sur votre droite, nous avons un diagramme en barres représentant le nombre d'abandons par pilote sur une saison. Remarquons que le nombre d'abandons n'est pas forcément lié au classement final. Les pilotes n'apparaissant pas sur le graphique sont ceux ayant débutés et finis tous les grands prix."
g9 <- "Ici vous trouverez un diagramme en barres représentant le nombre d'abandons par grand prix sur une saison. On pourrait se demander si les circuits ayant eu le plus d'abandons sont les plus dangereux. D'autres facteurs peuvent rentrer en jeu comme par exemple la météo."

g10 <- "Sur votre gauche, vous remarquerez un diagramme en barres. Ce diagramme représente la fréquence d'abandons de chaque pilote entre deux années choisies. Vous remarquerez que plus l'écart entre les deux années est grand, plus le graphique est dense. Ceci s'explique du fait de l'ajout de pilotes."

ui <- dashboardPage(
  #Tableau du haut
  header = dashboardHeader(title = tags$img(src='logof1_1.png', height = '45', width ='80'),# ou width ='90'
  ...
  sidebar = dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Abandons", tabName = "Abandon",icon = icon("car-crash")),
      menuItem("Comparaisons", tabName = "Comparaison",icon = icon("compress-alt")),
      menuItem("Carte", tabName = "Map",icon = icon("map-pin")),
      hr(),
      ...
      menuItem("Pour les curieux", tabName = "Curieux",icon = icon("question-circle"))
    )
  ),
  body = dashboardBody(
    #Thème général
    ...
    tabItems(
      ...
      tabItem(tabName = "Ecuries",fluidRow(column(width = 12, box(title = "Année", width = NULL, solidHeader = TRUE, status = "info",style="circle",selectInput("anneeecuries", label = NULL,choices = seq(2020,1950, by =-1)),selected ="choisir"))),
              fluidRow(column(width = 8, box(title = "Pourcentage de victoires par Ecurie",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("datateamanalyse"))),
      ...
      tabItem(tabName = "Abandon",fluidRow(column(width = 12, box(title = "Année", width = NULL, solidHeader = TRUE, status = "info",style="circle",selectInput("anneeabandon", label = NULL,choices = seq(2020,1950, by =-1)),selected ="choisir"))),
              fluidRow(column(width = 8, box(title = "Nombre d'abandons pour chaque Ecurie",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("dnfteam"))),
      ...
      tabItem(tabName = "Comparaison",fluidRow(column(width = 6, box(title = "Année n°1", width = NULL, solidHeader = TRUE,status = "info", style="circle",selectInput("anneecompar1", label = NULL,choices = seq(2019,1950, by =-1)),selected ="choisir")),
              ...
              fluidRow(column(width = 6, box(title = "Pourcentage de victoires par écurie",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("datateamanalyseA1"))),
                       column(width = 6, box(title = "Pourcentage de victoires par écurie",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("datateamanalyseA2")))),
              fluidRow(column(width = 9, box(title = "Fréquence d'abandons de chaque pilote entre deux années choisies",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("dnffrq"))),
                       column(width = 3, box(title = "Interprétation",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,g10))
              )
              
      ),
      tabItem(tabName = "Curieux", fluidRow(column(width = 12, tags$iframe(style="width: 100%", height= 800 ,src= "Help.html"))))
    )
  ),
