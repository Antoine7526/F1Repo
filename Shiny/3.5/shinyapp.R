...
g3 <- "Ce diagramme en barre représente le nombre moyen de points par pilote sur une saison. Remarquons qu'à l'aide de ce graphique nous pouvons retrouver un semblant du classement final des pilotes. La barre la plus haute représentant le leader."
...
g7 <- "Ce diagramme circulaire représente le pourcentage d'abandon pour chaque écurie lors d'une saison. Nous considérons comme abandon tous ceux n'ayant pas pris les départs mais également ceux qui n'ont pas terminé la course."
g8 <- "Sur votre droite, nous avons un diagramme en barre représentant le nombre d'abandons par pilote sur une saison. Remarquons que le nombre d'abandons n'est pas forcément lié au classement final. Les pilotes n'aparaissant pas sur le graphique sont ceux ayant débutés et finis tous les grands prix."
...
g10 <- "Sur votre gauche vous remarquerez un diagramme en barre. Ce diagramme représente la fréquence d'abandons de chaque pilote entre deux années choisies. Vous remarquerez que plus l'écart entre les deux années est grand plus le graphe est dense. Ceci s'explique du fait de l'ajout de pilotes. De plus, plus la barre sera haute plus le pilote a de chance de ne pas finir sa course."
...
ui <- dashboardPage(
  ...
  header = dashboardHeader(title = tags$img(src='logo.png', height = '50', width ='150'),
                           dropdownMenu(type = "messages", 
                                        ...
                                        badgeStatus =NULL,
                                        headerText = ("Cette Application est en cours de création. Dernière mise à jour réalisée le 23 nov. 2021"))
  ),
  #Tableau de gauche
  ...
  body = dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "Ecuries",fluidRow(column(width = 12, box(title = "Année", width = NULL, solidHeader = TRUE, status = "info",style="circle",selectInput("anneeecuries", label = NULL,choices = seq(2020,1950, by =-1)),selected ="choisir"))),
              fluidRow(column(width = 8, box(title = "Pourcentage de victoires par écurie",width = NULL,solidHeader = TRUE, status = "info",style="circle",label = NULL,plotlyOutput("datateamanalyse"))),
